...
def _cost_status_and_bar_seconds(database_file: str):
    ...
    the experiments table so the auditor can BLOCK on UNCALIBRATED costs and
    cluster events with the right bar size. Safe defaults if unavailable."""
...
def main():
    ...
    # Audit ONE run by default — the most recent — so we never audit a blended
    # mixture of every run ever stored (ChatGPT #15 / run-isolation).
    ...
    print(
        ...
        f"  (run_id={run_id}, cost_status={cost_status})"
    )
