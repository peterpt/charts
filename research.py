...
class Config:
    ...
    # Floor on the predicted move required to trade, applied even when cost is
    # UNCALIBRATED — prevents the degenerate "trade every bar" behavior that
    # makes the virtual signal meaningless. ~0.15% for 1h bars. Tunable.
    uncalibrated_decision_floor: float = 0.0015
...
def _parse_candle_timestamps(series):
    """Parse candle timestamps ROBUSTLY. Capital returns epoch SECONDS
    (e.g. 1786921200). A bare pd.to_datetime on integer input defaults to
    NANOSECONDS, mangling every timestamp to 1970 — which silently corrupts the
    market-hours filter and the forming-candle drop. Detect the epoch scale and
    convert correctly; fall back to string parsing for ISO inputs.
    """
    s = pd.Series(series)
    if pd.api.types.is_numeric_dtype(s):
        nums = pd.to_numeric(s, errors="coerce").dropna()
        v = float(nums.iloc[0]) if len(nums) else 0.0
...
def request_market_history(instrument: str, timeframe: str,
    ...
    if os.path.isfile(path):
        try:
            ...
            cached["timestamp"] = _parse_candle_timestamps(cached["timestamp"])
        except Exception:
            cached = None
    ...
    if len(fresh):
        fresh["timestamp"] = _parse_candle_timestamps(fresh["timestamp"])
        fresh = _drop_forming_candle(fresh, timeframe)
...
def _drop_forming_candle(df: pd.DataFrame, timeframe: str) -> pd.DataFrame:
    """Drop the most-recent candle if its period has NOT fully elapsed — i.e.
    it is still forming. Training on an incomplete bar is a look-ahead error:
    the model would see a partial OHLC as if final, and its forward-return
    target would be wrong. This enforces the frozen decision_timing=BAR_CLOSE
    invariant (ChatGPT candles flag). Also drops any candle timestamped in the
    future (clock skew) defensively.
    """
    if df is None or not len(df):
        return df
    tf = _TF_SECONDS.get(timeframe, 3600)
    now = pd.Timestamp.now(tz="UTC")
    last_ts = df["timestamp"].iloc[-1]
    # a candle labelled at time T covers [T, T+tf); it is CLOSED only once
    # now >= T + tf. If not, drop it (and any others not yet closed).
    closed_cutoff = now - pd.Timedelta(seconds=tf)
    keep = df["timestamp"] <= closed_cutoff
    dropped = int((~keep).sum())
    if dropped:
        # Only log when we actually drop something (usually the 1 forming bar).
        try:
            print(f"  [history] dropped {dropped} forming/unclosed candle(s) "
                  f"(BAR_CLOSE enforced)")
        except Exception:
            pass
...
def run_walk_forward(
    ...
    for position in range(
        ...
        # Skip invalid targets. The market-hours filter sets forward_return to
        # NaN when the holding window crosses a market closure (e.g. QTUMUSD's
        # daily 21:00-21:05 maintenance gap), and a NaN target cannot be scored
        # — executing on it produces NULL pnl/equity rows (the QTUMUSD 0.00
        # equity bug). These observations are simply not evaluable, so skip them
        # entirely (not a win, not a loss, not a NO_TRADE).
        import math as _math
        if _math.isnan(actual_return) or _math.isnan(entry_price):
            continue
        ...
        # edge_floor is the minimum predicted move required to trade. When cost
        # is UNCALIBRATED (0), cost+minimum_net_edge collapses to a tiny value,
        # so nearly every bar would trade — a degenerate signal (this produced
        # the ~9,300 trades on ~6,000 bars that ChatGPT flagged). To keep the
        # decision meaningful even before real costs exist, the floor never
        # drops below `uncalibrated_decision_floor`. This does NOT affect
        # qualification (the auditor judges real evidence separately) — it keeps
        # the virtual/observability signal sane and honest.
        edge_floor = max(
            transaction_cost + config.minimum_net_edge,
            getattr(config, "uncalibrated_decision_floor", 0.0015),
        )
